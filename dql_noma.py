...
def calculate_rsi(prices, window=14):
    ...
    avg_gain = np.concatenate([np.full(window-1, avg_gain[0]), avg_gain])
    avg_loss = np.concatenate([np.full(window-1, avg_loss[0]), avg_loss])
...
class StockTradingEnv:
    def __init__(self, prices, volumes, dates,
        ...
        self.volume = volumes / np.max(volumes)
        self.dates = dates
    ...
    def _get_state(self):
        ...
        return np.concatenate([price_window, rsi_window, volume_window,
...
def train_and_test_dqn():
    ...
    state_size = train_env.window_size * 3 + 2
    action_size = 3
    agent = DQNAgent(17, action_size)

    episodes = 100
    batch = 32
    best_pv = 0
    model_path = "2330_dqn_rsi_best.pth"
    ...
    out_path = "C:/Users/user/Desktop/2330_rsi_test_results.csv"
