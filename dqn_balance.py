...
class StockTradingEnv:
    def __init__(self, prices, volumes, dates,
                 window_size=5, initial_balance=4500, trading_cost=0.001):
    ...
    def step(self, action):
        ...
        if action == 1 and self.trade_cooldown == 0:
            ...
            revenue = sold_shares * current_price * (1 - self.trading_cost)
            buy_price = self.last_buy_price if self.last_buy_price else current_price
...
def train_and_test_dqn():
    ...
    for e in range(episodes):
        ...
        if train_env.balance > best_balance:
            best_balance = train_env.balance
            agent.save_model(model_path)

        print(f"Ep {e+1}/{episodes} | Reward {total_reward:.2f} | Cash {train_env.balance:.2f} | "
    ...
    results = []
    test_env = StockTradingEnv(prices[test_mask], volumes[test_mask], dates[test_mask], window_size=5)
    agent.load_model(model_path)
    state = test_env.reset()
    ...
    # 圖片畫出價格、資產與現金走勢，並標出買賣點
    import matplotlib.pyplot as plt

    plt.figure(figsize=(14, 8))
    plt.plot(df['Date'], df['Price'], label='Stock Price', color='blue')
    plt.plot(df['Date'], df['Portfolio_Value'], label='Portfolio Value', color='green', linestyle='--', alpha=0.7)
    plt.plot(df['Date'], df['Balance'], label='Cash Balance', color='purple', linestyle=':', alpha=0.7)

    buy_df = df[df['Action'].str.startswith("Buy")]
    sell_df = df[df['Action'].str.startswith("Sell")]
    plt.scatter(buy_df['Date'], buy_df['Price'], marker='^', color='red', label='Buy', zorder=5)
    plt.scatter(sell_df['Date'], sell_df['Price'], marker='v', color='orange', label='Sell', zorder=5)

    plt.title('Stock Price, Portfolio Value, and Actions Over Time')
    plt.xlabel('Date')
    plt.ylabel('Value (Price / TWD)')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()

    image_path = r"C:\Users\user\Documents\Modern AI final project\2330_test_plot.png"
    plt.savefig(image_path)
    plt.close()

    out_path = r"C:\Users\user\Documents\Modern AI final project\2330_test_results.csv"
    df.to_csv(out_path, index=False)
    print(f"測試結果已儲存：{out_path}")
