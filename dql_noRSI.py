...
def calculate_technical_indicators(prices, volumes, window_short=5, window_long=10):
    ...
    ma_short = np.concatenate([np.full(window_short-1, ma_short[0]), ma_short])
    ma_long = np.convolve(prices, np.ones(window_long)/window_long, mode='valid')
    ma_long = np.concatenate([np.full(window_long-1, ma_long[0]), ma_long])
    volume_norm = volumes / np.max(volumes)
    return ma_short, ma_long, volume_norm
...
class StockTradingEnv:
    def __init__(self, prices, volumes, dates,
        ...
        self.ma5, self.ma10, self.volume = calculate_technical_indicators(prices, volumes, window_size)
        self.dates = dates
    ...
    def _get_state(self):
        ...
        ma5_window = self.ma5[s:e] / np.max(self.ma5)
        ma10_window = self.ma10[s:e] / np.max(self.ma10)
        volume_window = self.volume[s:e]
        return np.concatenate([price_window, ma5_window, ma10_window, volume_window,
...
def train_and_test_dqn():
    ...
    for e in range(episodes):
        ...
        print(f"Ep {e+1}/{episodes} | Reward {total_reward:.2f} | PV {pv:.2f} | Profit {profit:.2f} | ε={agent.epsilon:.2f}")
    ...
    print(f"動作統計：Hold={sum(r['Action']=='Hold' for r in results)}, Buy={sum(r['Action'].startswith('Buy') for r in results)}, Sell={sum(r['Action'].startswith('Sell') for r in results)}")
