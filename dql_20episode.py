...
def train_and_test_dqn():
    ...
    test_env_data = (prices[test_mask], volumes[test_mask], dates[test_mask])
    ...
    for e in range(episodes):
        ...
        if (e + 1) % 20 == 0:
            test_env = StockTradingEnv(*test_env_data, window_size=5)
            ...
            print(f"[Test @ Ep {e+1}] Portfolio Value = {info['portfolio_value']:.2f}")
    ...
    if test_history:
        ...
        plt.plot(df_test['Episode'], df_test['Portfolio_Value'], marker='o')
        plt.title('Test Portfolio Value Every 20 Episodes')
        plt.xlabel('Episode')
        plt.ylabel('Portfolio Value (TWD)')
        plt.grid(True)
        plt.tight_layout()
        plt.savefig("C:/Users/user/Desktop/2330_test_pv_plot.png")
        plt.show()
